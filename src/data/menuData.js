...
export const filterMenuItems = ({ category = 'all', diet = 'all', name = '' } = {}) => {
  const normalizedCategory = category.toLowerCase()
  const normalizedDiet = diet.toLowerCase().replace('-', '')
  const normalizedName = name.trim().toLowerCase()

  return menuItems.filter((item) => {
    const matchesCategory =
      normalizedCategory === 'all' || item.category === normalizedCategory
    const matchesDiet =
      normalizedDiet === 'all' ||
      (normalizedDiet === 'veg' ? item.isVeg : !item.isVeg)
    const matchesName =
      !normalizedName || item.name.toLowerCase().includes(normalizedName)
